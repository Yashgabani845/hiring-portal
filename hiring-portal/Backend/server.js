...
app.post("/api/SendResetEmail", async (req, resp) => {
  ...
  const mailOptions = {
    ...
    <p>Reset your password from the link below.</p>
     <a href="your hosted site address/ForgotPassword/${UserEmail}"><button>Click here</button></a> to reset password`,
  };
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      console.log("Email sent: " + info.response);
      resp.status(200).send({ message: "Password reset email sent", user });
    }
  });
});

// api to reset password directly
app.post("/api/DirectResetPassword/:email", async (req, resp) => {
  try {
